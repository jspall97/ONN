...
slm_R1_w = 300
slm_sig_w = slm_w - int(slm_R1_w * 1.05)
# slm_R2_h = 200
slm_label_h = 50
slm_sig_h = 700
...
slm_R1_s = np.s_[slm_x0+slm_w-slm_R1_w:slm_x0+slm_w, slm_yc-slm_R1_w//2:slm_yc+slm_R1_w//2]

slm_label_s = np.s_[slm_x0:slm_x0+slm_sig_w, slm_y0:slm_y0+slm_label_h]
slm_sig_s = np.s_[slm_x0:slm_x0+slm_sig_w, slm_y0+slm_h-slm_sig_h:slm_y0+slm_h]
...
dmd_R1_w = int(slm_R1_w * dmd_w/slm_w)

dmd_label_h = int(slm_label_h * dmd_h/slm_h)

dmd_s = np.s_[dmd_x0:dmd_x0+dmd_w, dmd_y0:dmd_y0+dmd_h]

dmd_R1_s = np.s_[dmd_x0+dmd_w-dmd_R1_w:dmd_x0+dmd_w, dmd_yc-dmd_R1_w//2:dmd_yc+dmd_R1_w//2]

dmd_label_s = np.s_[dmd_x0:dmd_x0+dmd_sig_w, dmd_y0:dmd_y0+dmd_label_h]
dmd_sig_s = np.s_[dmd_x0:dmd_x0+dmd_sig_w, dmd_y0+dmd_h-dmd_sig_h:dmd_y0+dmd_h]
...
gr_ref = 2*np.pi*(-2*X)/period+1e-5
...
def update_params(_n, _m, batch_size, num_frames):
    ...
    dmd_block_w = int((slm_block_w-2) * dmd_w / slm_w)
...
def make_slm1_rgb(target, R1_ampl=0., R1_phase=0):
    ...
    # wf correct
    gpu_phi[slm_R1_s] += gpu_slm_wf_temp[slm_R1_s]
...
def make_dmd_image(arr, R1_ampl=0):
    ...
    for j in range(dmd_block_h // 2):
        mapped[ws_local, dmd_ycc - j, dmd_xcc] = mask
        mapped[ws_local, dmd_ycc + j + 1, dmd_xcc] = mask
    ...
    img = cp.zeros((dmd_w, dmd_h), dtype=cp.uint8)
    img[dmd_sig_s] = out.copy().T
    ...
    return rgb.astype(cp.uint8), img


def make_dmd_batch(vecs, batch_size, num_frames):
